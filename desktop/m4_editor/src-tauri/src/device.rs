...
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct ScreensaverResult {
    pub bytes_sent: usize,
    pub resumed_at: u32,
}

fn status_detail(status: u32, value: u32) -> String {
    match status {
        1 => "bad frame or checksum".into(),
        2 => "transfer is not open".into(),
        3 => "microSD I/O failure".into(),
        4 => "bundle validation failed".into(),
        5 => "device is busy".into(),
        6 => match value {
...
fn parse_response(bytes: &[u8], opcode: u8, sequence: u32) -> Result<(u32, u32), DeviceError> {
    ...
    if u32::from_le_bytes(bytes[16..20].try_into().unwrap()) != crc32(&bytes[20..28]) {
        return Err(DeviceError::Protocol("response checksum mismatch".into()));
    }
    Ok((
...
fn checked_exchange_with_timeout(
    session: &mut transport::Session,
    opcode: u8,
    sequence: u32,
    payload: &[u8],
...
pub fn test_screensaver() -> Result<(), DeviceError> {
    let mut session = transport::Session::open()?;
    let capabilities = checked_exchange(&mut session, HELLO, 1, &[], "capability query")?;
...
pub fn sync(bundle: &[u8], fingerprint: String) -> Result<SyncResult, DeviceError> {
    ...
    begin.extend_from_slice(&payload_crc.to_le_bytes());
    let resumed_at = checked_exchange(&mut session, BEGIN, 3, &begin, "begin upload")?;
    if resumed_at as usize > bundle.len() {
        return Err(DeviceError::Protocol(format!(
            "resume offset {resumed_at} exceeds bundle size {}",
            bundle.len()
        )));
    }
        ...
        let acknowledged =
            match checked_exchange(&mut session, CHUNK, sequence, &payload, "upload chunk") {
                ...
                Err(DeviceError::Protocol(message)) if message.contains("timed out") => {
                    ...
                    sequence += 1;
                    let safe_offset = checked_exchange(
                        &mut session,
                        BEGIN,
                        sequence,
                        &begin,
    ...
    Ok(SyncResult {
        generation,
        bytes_sent: bundle.len() - resumed_at as usize,
        resumed_at,
        fingerprint,
    })
}
...
pub fn upload_screensaver(media: &[u8]) -> Result<ScreensaverResult, DeviceError> {
    ...
    let resumed_at = checked_exchange(
        ...
        2,
        &begin,
        "begin screensaver upload",
    )
    .map_err(|error| DeviceError::Protocol(format!("screensaver begin: {error}")))?;
    if resumed_at as usize > media.len() {
        return Err(DeviceError::Protocol(format!(
            "resume offset {resumed_at} exceeds media size {}",
            media.len()
        )));
    }
    let mut offset = resumed_at as usize;
        ...
        while offset < batch_end {
            ...
            if capabilities & MEDIA_BATCH_CAPABILITY != 0 && !is_batch_end {
                session
                    ...
                    .map_err(|error| {
                        DeviceError::Protocol(format!("screensaver chunk at {offset}: {error}"))
                    })?;
            } else {
                ...
                if status != 0 {
                    return Err(DeviceError::Rejected {
                        operation: "upload screensaver batch",
                        status,
                        detail: status_detail(status, acknowledged),
                    });
                }
                if acknowledged as usize != end {
    ...
    let received = checked_exchange(
        &mut session,
        MEDIA_BEGIN,
        sequence,
        &begin,
        "verify screensaver stream",
    )?;
    sequence += 1;
    if received as usize != media.len() {
        return Err(DeviceError::Protocol(format!(
            "device received {received} screensaver bytes; expected {}",
            media.len()
        )));
    }
    ...
    Ok(ScreensaverResult {
        bytes_sent: media.len() - resumed_at as usize,
        resumed_at,
    })
}
...
mod transport {
    ...
    const INTERFACE_GUID: Guid = Guid {
        ...
        data2: 0x7E95,
        data3: 0x4F15,
        data4: [0xA9, 0xFD, 0x7B, 0xBC, 0x31, 0xE4, 0xF0, 0xC4],
    };
    #[repr(C)]
    ...
    unsafe fn device_path() -> Result<Vec<u16>, DeviceError> {
        ...
        if set.0 == INVALID_HANDLE {
            return Err(DeviceError::NotFound);
        }
        let mut data: InterfaceData = zeroed();
        data.cb_size = size_of::<InterfaceData>() as u32;
        if SetupDiEnumDeviceInterfaces(set.0, null_mut(), &INTERFACE_GUID, 0, &mut data) == 0 {
            return Err(DeviceError::NotFound);
        }
    ...
    impl Session {
        pub fn open() -> Result<Self, DeviceError> {
            unsafe { Connection::open().map(Self) }
        }
...
mod transport {
    ...
    impl Session {
        ...
        pub fn send(&mut self, _: &[u8]) -> Result<(), DeviceError> {
            Err(DeviceError::NotFound)
        }
        pub fn exchange(&mut self, _: &[u8]) -> Result<Vec<u8>, DeviceError> {
            Err(DeviceError::NotFound)
        }
        pub fn exchange_with_timeout(&mut self, _: &[u8], _: u32) -> Result<Vec<u8>, DeviceError> {
            Err(DeviceError::NotFound)
        }
    }
}
